...
$r->free_result();	// leberta a memória do resultado ocupado por $r
$dbc->close();
...
if ($paginas > 1) 
{
	...
	{
		echo '<a href="ver_utilizadores.php?s=' . ($inicio - $reg_por_pagina) . '&p=' . $paginas  . '">Anteriores</a> ';
	}
	...
	for ($i = 1; $i <= $paginas; $i++) 
	{
		...
		{
			echo '<a href="ver_utilizadores.php?s=' . (($reg_por_pagina * ($i - 1))) . '&p=' . $paginas  . '">' . $i . '</a> ';
		} else {
			echo $i . ' ';
		}
	}
	...
	{
		echo '<a href="ver_utilizadores.php?s=' . ($inicio + $reg_por_pagina) . '&p=' . $paginas . '">Próximas</a>';
	}
