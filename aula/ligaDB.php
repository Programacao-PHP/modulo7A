<?php
	...
	*/
    ...
    define('DB_NAME', 'contactos');		// nome da base de dados

	// Ligação ao servidor MySQL
	// O símbolo @ evita detalhes das mensagens de erro
    $liga = @mysqli_connect(DB_HOST, DB_USER, DB_PASSWORD) OR die('Problemas na ligação ao servidor MySQL: ' . mysqli_connect_error());

	// seleciona a base de dados no servidor
	mysqli_select_db($liga, DB_NAME) or die("A base de dados não existe");

	mysqli_set_charset($liga, "utf8");	// permite corrigir os caracteres com cedilhas na visualização dos campos
